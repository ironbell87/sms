...
const g_spt = ["Simple support", "Cantilever"];
let g_setting = { b: 30.0, h: 50.0, L: 600.0, P: 100.0, E: 2200.0, S: "Simple support" };
...
$(document).ready(function () {
    ...
    $(document).on("input", "#input_S", function () {
        ...
        g_setting.S = g_spt[spt_idx];
        if (g_setting.S == g_spt[0]) g_loc_fr = g_loc_to = g_span / 2; // in simple beam, load at beam center
        else g_loc_fr = g_loc_to = g_span; // in cantilever beam, load at beam tip
        $("#label_S").html(g_setting.S);
        draw_problem();
        draw_measurement();
    });
...
function draw_problem() {
    if (g_setting.S == g_spt[0]) draw_simple_beam_problem();
...
function draw_measurement() {
    ...
    var crds = get_deflections(xxs);
    var defl_max = Math.max(...crds); // ... spread operator : spread all the element
    ...
    // path string
    var defl_path = "M0,0";
    crds.forEach(function (yy, i) { defl_path += "L" + g_beam_scaler(xxs[i]) + "," + g_defl_scaler(yy); });
    g_deflection.append("path")
    ...
    // draw scale
    g_deflection.append("g").classed("major_x_axis", true)
        .call(d3.axisTop(g_beam_scaler).ticks(6).tickSize(10));
    g_deflection.append("g").classed("minor_x_axis", true)
        .call(d3.axisBottom(g_beam_scaler).ticks(120).tickSize(5).tickPadding(600));
    var d3_y_axis = g_deflection.append("g").classed("y_axis", true)
        .attr("transform", "translate(" + g_scale_x + ", 0)");
    d3_y_axis.append("g").classed("major_y_axis", true)
...
function get_slope(p_xx) {
    // coefficients for slope curve
    var cfs = [-1 / 2, g_setting.L, 0]; // cantilever
    if (g_setting.S == g_spt[0]) cfs = [-1 / 4, 0, Math.pow(g_setting.L, 2) / 16]; // simple beam
    ...
    // get slope
    if (g_setting.S == g_spt[0]) { // simple beam
...
function get_deflection(p_xx) {
    ...
    if (g_setting.S == g_spt[0]) cfs = [-1 / 12, 0, Math.pow(g_setting.L, 2) / 16, 0]; // simple beam
...
function get_deflections(p_xxs) {
    ...
    //// get deflection
    //if (g_setting.S == g_spt[0]) return poly_vals(cfs, new_xxs); // simple beam
